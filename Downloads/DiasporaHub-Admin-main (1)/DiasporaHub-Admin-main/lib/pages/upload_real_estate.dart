  ...
  Future<void> _submit() async {
    ...
    if (_pickedImageBytes == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please upload an image for the property'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      if (_pickedPdfBytes != null && _pickedPdfName != null) {
        ...
        final extension = _pickedPdfName!.split('.').isNotEmpty
            ...
            : 'pdf';
        final pdfPath =
            'property_pdfs/${DateTime.now().millisecondsSinceEpoch}.$extension';
      ...
      String listingTypeDB = _listingType == 'For Sale' ? 'sale' : 'rent';
      ...
      if (mounted) {
        ...
        debugPrint(
          'Upload succeeded but widget is no longer mounted; skipping UI update.',
        );
      }
    } catch (e, st) {
      ...
      if (mounted) {
        ...
        debugPrint(
          'Could not show error to user because widget is unmounted: $userMessage',
        );
      }
    } finally {
      if (mounted) {
                            ...
                            Expanded(
                              child: DropdownButtonFormField<int>(
                                ...
                                items: List.generate(10, (i) => i + 1)
                                    ...
                                    .toList(),
                                onChanged: (v) =>
                                    setState(() => _bedrooms = v ?? 1),
                                validator: (v) =>
                                    v == null ? 'Please select bedrooms' : null,
                              ),
                            ),
                            const SizedBox(width: 12),
                            Expanded(
